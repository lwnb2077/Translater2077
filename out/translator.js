...
exports.CodeTermDictionary = exports.DeepLTranslator = exports.GeminiTranslator = exports.OpenAITranslator = exports.GoogleTranslator = void 0;
const axios_1 = __importDefault(require("axios"));
class GoogleTranslator {
    constructor(apiKey) {
        this.name = 'google';
        this.apiKey = ''; // 用户需要在设置中配置API密钥
    ...
    // 备用翻译方案，使用多个免费翻译服务
    async translateFallback(text, targetLang, sourceLang = 'auto') {
        // 尝试多个翻译服务
        const services = [
    ...
    async translateWithGoogleWeb(text, targetLang, sourceLang) {
    ...
    async translateWithMyMemory(text, targetLang, sourceLang) {
        ...
        const normalizeForMyMemory = (lang, isSource) => {
            if (!lang)
                return '';
            if (lang === 'auto')
                return isSource ? 'en' : 'zh-CN';
            const lower = lang.toLowerCase();
            if (lower === 'zh-cn' || lower === 'zh_cn')
                return 'zh-CN';
            if (lower === 'zh-tw' || lower === 'zh_tw')
    ...
    translateOffline(text, targetLang) {
        ...
        const translatedWords = words.map(word => CodeTermDictionary.translateTerm(word.replace(/[^a-zA-Z]/g, '')));
        const hasTranslation = translatedWords.some((word, index) => word !== words[index].replace(/[^a-zA-Z]/g, ''));
        if (hasTranslation) {
    ...
    async testConnection() {
        try {
            if (this.apiKey) {
                // 使用不计费的语言列表端点校验密钥有效性
                const resp = await axios_1.default.get('https://translation.googleapis.com/language/translate/v2/languages', {
    ...
    async testConnection() {
        try {
            if (!this.apiKey)
                return false;
            // 仅请求模型元数据，零成本校验权限
            const resp = await axios_1.default.get(`https://api.openai.com/v1/models/${this.model}`, {
                headers: { 'Authorization': `Bearer ${this.apiKey}` }
            });
            return !!(resp.status >= 200 && resp.status < 300);
        }
...
class DeepLTranslator {
    ...
    async translate(text, targetLang = 'zh', sourceLang = 'auto') {
        ...
        // DeepL 语言代码映射
        const mapLang = (lang, isTarget = false) => {
            const lower = lang.toLowerCase();
            // 源语言 auto 对应 DeepL 的空值（自动检测）
            if (!isTarget && (lang === 'auto' || lang === ''))
                return undefined;
            // 中文简体/繁体
    ...
    async testConnection() {
        try {
            ...
            const resp = await axios_1.default.get(usageUrl, {
                headers: {
                    'Authorization': `DeepL-Auth-Key ${this.apiKey}`
                }
            });
...
// 编程术语词典
class CodeTermDictionary {
    static translateTerm(term, targetLang = 'zh-CN') {
        const lowerTerm = term.toLowerCase();
        const langMap = this.termsByLang[targetLang] || this.termsByLang['zh-CN'];
        return (langMap && langMap[lowerTerm]) || term;
    }
    static enhanceTranslation(text, translation, targetLang = 'zh-CN') {
        // 应用户要求：不再在结果末尾附加“编程术语解释”段落。
        // 直接返回原始翻译。
        return translation;
    }
    static translateUsingDictionary(text, targetLang = 'zh-CN') {
        if (!text)
            return undefined;
...
exports.CodeTermDictionary = CodeTermDictionary;
// multi-language glossary; default zh-CN
CodeTermDictionary.termsByLang = {
    'zh-CN': {
        'function': '函数',
        'variable': '变量',
        'constant': '常量',
