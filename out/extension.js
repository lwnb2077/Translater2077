...
const wordHelper_1 = require("./wordHelper");
...
function getUiStrings() {
    ...
    const lang = (cfg.get('targetLanguage', 'zh-CN') || '').toLowerCase();
    // 根据目标语言选择合适的UI语言
    if (lang === 'zh-cn')
        return UI_TEXT['zh-CN']; // 简体中文
    if (lang === 'zh-tw')
        return UI_TEXT['zh-TW']; // 繁体中文
    if (lang === 'ja')
        return UI_TEXT.ja; // 日语
    if (lang === 'ko')
        return UI_TEXT.ko; // 韩语
    if (lang === 'fr')
        return UI_TEXT.fr; // 法语
    if (lang === 'de')
        return UI_TEXT.de; // 德语
    return UI_TEXT.en; // 其他语言回退英文
}
let statusBarItem;
let settingsButton; // 仅在无翻译结果时显示
...
let lastOriginalForDetails = '';
let lastTranslatedForDetails = '';
let lastClickTime = 0;
...
function activate(context) {
    ...
    translationManager = new translationManager_1.TranslationManager();
    // 已移除输出面板历史记录功能
    // 更新翻译提供商
    const provider = config.get('apiProvider', 'google');
    ...
    const selectionChangeListener = vscode.window.onDidChangeTextEditorSelection(async (e) => {
        ...
        const autoTranslate = currentConfig.get('autoTranslate', true);
        if (!autoTranslate) {
            console.log('Auto translate disabled, skipping');
            return;
        }
        // 实时读取其他配置项
        const delay = currentConfig.get('translationDelay', 250);
        const selectionTranslate = currentConfig.get('selectionTranslate', true);
        const clickTranslateMode = currentConfig.get('clickTranslateMode', 'single');
        const singleClickTranslate = clickTranslateMode === 'single';
        const doubleClickTranslate = clickTranslateMode === 'double';
        const minWordLength = currentConfig.get('minWordLength', 2);
            ...
            if (shouldProcess) {
                ...
                cursorTimeout = setTimeout(async () => {
                    try {
                        ...
                        // 使用智能单词获取
                        const smartWord = wordHelper_1.WordHelper.getSmartWordAtPosition(editor.document, selection.active);
                        console.log(`Smart word detected: "${smartWord}"`);
                        if (smartWord && wordHelper_1.WordHelper.shouldTranslate(smartWord, minWordLength, true)) {
                            console.log(`Should translate: ${smartWord}`);
                            // 避免重复翻译同一个单词
                            if (smartWord !== lastTranslatedText) {
                                // 提取可翻译文本
                                const translatableText = wordHelper_1.WordHelper.extractTranslatableText(smartWord);
    ...
    const showDetailsCommand = vscode.commands.registerCommand('codeTranslator.showDetails', async () => {
        ...
        const config = vscode.workspace.getConfiguration('codeTranslator');
        const mode = (config.get('detailsDisplayMode', 'system') || 'system').toLowerCase();
        ...
        if (mode === 'system') {
            ...
            const picked = await vscode.window.showInformationMessage(message, { modal: true }, btnClose, btnOpenSettings, ...(btnViewFull ? [btnViewFull] : []), btnCopyBoth, btnCopyTranslation, btnCopyOriginal);
            if (!picked || picked === btnClose)
                return;
            ...
            if (btnViewFull && picked === btnViewFull) {
                // 直接打开面板模式展示完整内容
                const width = Math.max(480, Math.min(1600, config.get('detailsPanelWidth', 800)));
                const panel = vscode.window.createWebviewPanel('codeTranslator.details', ui.detailsTitle, vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: true });
  ...
  <script>
    ...
    const configChangeListener = vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration('codeTranslator')) {
            ...
            const provider = updatedConfig.get('apiProvider', 'google');
...
async function translateTextWithOriginal(text, originalText) {
    if (!text || text.trim().length === 0) {
        return;
    }
    ...
    const maxTextLength = configForLimit.get('maxTextLength', 2000);
    const ui = getUiStrings();
...
function displayTranslation(translatedText, originalText) {
    // 检查是否有有效的翻译结果
    const ui = getUiStrings();
    ...
    for (let i = 0; i < takeLines; i++) {
        ...
        if (seg.length > perSeg)
            seg = seg.slice(0, perSeg) + '…';
        segments.push(`- ${seg}`);
    }
    let composed = segments.join('  ');
    if (composed.length > maxLength)
        composed = composed.slice(0, maxLength) + '…';
    // 在状态栏显示“多段”文本
    ...
    const shortenOriginalForTooltip = (s) => s.length > 50 ? (s.slice(0, 20) + '…' + s.slice(-18)) : s;
    const providerInfo = providerTag ? `\n\n${ui.sourceLabel || '来源'}：${providerTag}` : '';
    // 先显示译文，再显示原文
    const translatedForTooltip = truncateMultiline(translatedText
    ...
    // 获取自动消失设置
    const autoHide = config.get('autoHideTranslation', true);
    const autoHideDelay = config.get('autoHideDelay', 10);
    // 只有在启用自动消失时才设置定时器
    if (autoHide) {
        // 根据翻译类型设置不同的显示时间
